...
Widget build(BuildContext context) {
  return Scaffold(
    ...
    // REMOVED SingleChildScrollView and SafeArea here for clarity
    body: !isUserDataLoaded || Cid == null
        ? const Center(child: CircularProgressIndicator()) // Wait for Cid to load
        : StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance
                .collection("communities")
                .doc(Cid) // Now we are sure Cid is not null
                .collection("loans")
            ...
            builder: (context, snapshot) {
              ...
              if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                return const Center(child: Text("No transactions till now"));
              }

              // REMOVED Expanded. ListView.builder is the top-level widget here.
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  // Note: data["duration_value"] is likely an int or String, 
                  // ensure .toString() if you force it to String.
                  String duration_value = data["duration_value"]?.toString() ?? "0";
                  String duration_unit = data["duration_unit"] ?? "";

                  return ListTile(
                    title: Text("$owner to $requester for $duration_value $duration_unit"),
                    onTap: () => show_t_Details(context, data),
                  );
                },
              );
            },
